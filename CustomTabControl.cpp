...
void CustomTabControl::SwitchTabOrder(int index1, int index2) {
	...
	if (index1 < index2) {
		m_tabTitles.insert(m_tabTitles.begin() + index2 - 1, draggedTabTitle);
	}
	else {
...
void CustomTabControl::OnPaint(HWND hWnd) {
	...
	for (size_t i = 0; i < m_tabTitles.size(); ++i) {
		...
		tabRects[i] = { xPos, 0, xPos + tabWidths[i], tabHeight };
...
void CustomTabControl::OnLButtonDown(HWND hWnd, int x, int y) {
	...
	if (index != -1) {
		if (isClose) {
			...
			SetCapture(hWnd); // 閉じるボタンのドラッグを追跡するためにキャプチャ
		}
		else {
			m_selectedTab = index;
